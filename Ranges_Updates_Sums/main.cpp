...
public:
    ...
    void update_2(long long l, long long r, int p, int i, int j, long long val) {
        lazy_propagation(l, r, p);
        if (l >= i && r <= j) {
            st[p].sum = (r - l + 1) * val;
            st[p].settle = 0;
        ...
        update_2(l, (l + r) / 2, 2 * p, i, j, val);
        update_2((l + r) / 2 + 1, r, 2 * p + 1, i, j, val);
        merge(p, 2 * p, 2 * p + 1);
        return;
    }
